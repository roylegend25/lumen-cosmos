  ...
  if (!constellation) {
    return (
      <div className="min-h-screen pt-32 section-padding text-center">
        ...
        <Link to="/constellations" className="btn-secondary mt-4 inline-flex">Back</Link>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen pt-24 pb-16">
      <div className="section-padding container-wide">
        <Link to="/constellations" className="inline-flex items-center gap-2 text-sm text-cosmos-silver hover:text-cosmos-pure mb-8 transition-colors">
          <ArrowLeft className="w-4 h-4" /> All constellations
        </Link>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="grid lg:grid-cols-2 gap-12 mb-16">
          <div>
            <div className="flex items-center gap-3 mb-4">
              <h1 className="text-display-sm md:text-display-md font-semibold text-cosmos-pure">{constellation.name}</h1>
              <span className="font-mono text-sm text-cosmos-glow bg-cosmos-accent/10 px-3 py-1 rounded-full">{constellation.abbreviation}</span>
            </div>
            <p className="text-cosmos-silver leading-relaxed mb-8">{constellation.description}</p>
            <div className="grid grid-cols-2 gap-4">
              <div className="card p-4">
                <div className="flex items-center gap-2 text-cosmos-silver text-xs mb-1"><Eye className="w-3.5 h-3.5" /> Visibility</div>
                <p className="text-sm text-cosmos-pure">{constellation.visibility}</p>
              </div>
              <div className="card p-4">
                <div className="flex items-center gap-2 text-cosmos-silver text-xs mb-1"><MapPin className="w-3.5 h-3.5" /> Hemisphere</div>
          ...
          <div className="card relative overflow-hidden min-h-[340px] rounded-2xl">
            <Suspense fallback={<div className="absolute inset-0 bg-cosmos-night" />}>
              <StarFieldCanvas count={900} radius={50} depth={40} size={0.14} color="#d4c8ff" speed={0.00025} className="opacity-95" />
            </Suspense>
            <div className="absolute inset-0 bg-gradient-to-t from-cosmos-void/80 via-transparent to-transparent pointer-events-none" />
        ...
        {relatedStars.length > 0 && (
          <section>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {relatedStars.map((s) => (
                <Link key={s.id} to={`/stars/${s.id}`} className="card p-5 group hover:border-cosmos-accent/30">
                  ...
                  <div className="flex gap-3 text-xs text-cosmos-silver">
                    <span>mag {s.magnitude}</span><span>{s.spectralType}</span><span>{s.distance} ly</span>
                  </div>
                </Link>
              ))}
            </div>
