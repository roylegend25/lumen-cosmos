...
export function StarDetail() {
  ...
  if (!star) {
    return (
      <div className="min-h-screen pt-32 section-padding text-center">
        ...
        <Link to="/stars" className="btn-secondary mt-4 inline-flex">Back</Link>
      </div>
    )
  }
  ...
  if (star.luminosity) stats.push({ label: 'Luminosity', value: `${star.luminosity.toLocaleString()} L\u2609` })
    ...
    <div className="min-h-screen pt-24 pb-16 relative">
      <div className="fixed inset-0 pointer-events-none opacity-40 -z-10">
        <Suspense fallback={null}>
          <StarFieldCanvas count={1200} radius={80} depth={60} size={0.09} speed={0.00008} />
        </Suspense>
      </div>

      <div className="section-padding container-wide relative z-10">
        <Link to="/stars" className="inline-flex items-center gap-2 text-sm text-cosmos-silver hover:text-cosmos-pure mb-8 transition-colors">
          <ArrowLeft className="w-4 h-4" /> All stars
        </Link>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          {constellation && (
            <Link to={`/constellations/${constellation.id}`} className="text-sm text-cosmos-glow hover:underline mb-2 inline-block">
              {constellation.name}
            </Link>
          )}
          <h1 className="text-display-sm md:text-display-md font-semibold text-cosmos-pure mb-2">{star.name}</h1>
          ...
          <div className="card p-6 mb-8">
            ...
            <div className="grid sm:grid-cols-2 gap-4 font-mono text-sm">
              <div><span className="text-cosmos-silver">RA </span><span className="text-cosmos-pure">{star.coordinates.ra}</span></div>
              <div><span className="text-cosmos-silver">Dec </span><span className="text-cosmos-pure">{star.coordinates.dec}</span></div>
            </div>
          </div>
